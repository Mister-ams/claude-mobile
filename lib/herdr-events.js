 ...
 * markSeen() is reported as `done`; markSeen() turns it into `idle`. The
 * last raw status survives a dropped SUBSCRIPTION (herdr still answering, so
 * the poll and the resubscribe read the same agents): a turn that ends while
 * the pipe is down is still an unseen finish. It is forgotten only when herdr
 * itself cannot answer, since a restarted herdr's first reading says nothing
 * about the turn that was running before. herdr's
...
function createAgentFeed(opts) {
  ...
  function emit() {
    ...
    // lastRaw is kept across a dropped subscription and cleared by
    // herdrUnreachable() (see the header).
  ...
  // herdr did not answer a snapshot: it may be restarting, so the next
  // reading is a first reading again and a restart is not mistaken for a
  // finish. A dropped subscription alone does not come here.
  function herdrUnreachable() { lastRaw = null; }
  ...
  function startPoll() {
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (feed !== 'events') { feed = 'down'; herdrUnreachable(); }
      }
      emit();
