...
//
// T08: `focus` is DEC mode 1004 (focus reporting). It rides in the same value
// because it is plumbed the same way -- read off the mirror, pushed on each
// transition, and the server writes CSI I / CSI O only while it is on. herdr
// asks for it so it can redraw when its terminal regains focus, and forwards
// the report to a pane that asked in turn. Same asymmetry as above: no
// mirror means off, never a guess.
function mouseState(session) {
  const term = session && session.headless;
  if (!term) return { tracking: 'none', encoding: 'default', focus: false };
  let tracking = 'none';
  try { tracking = term.modes.mouseTrackingMode || 'none'; }
  catch (e) { tracking = 'none'; }
  let focus = false;
  try { focus = !!term.modes.sendFocusMode; }
  catch (e) { focus = false; }
  return { tracking, encoding: session.mouseEncoding || 'default', focus };
}
