...
module.exports = function createServerControl({ config, audit, installDir }) {
  ...
  // PM2 exports the app name as `name` and its slot as `pm_id`. The name is
  // what update.sh restarts.
  //
  // There is deliberately NO fallback to 'claude-mobile'. That guess is the
  // wrong-process bug wearing a default: an instance that cannot identify
  // itself would tell update.sh to restart whatever is called claude-mobile,
  // which on this machine is the LIVE service. Not knowing is a reason to
  // refuse, not a reason to pick the most dangerous answer.
  const PM2_NAME = config.pm2Name || process.env.name || null;

  // Both controls assume something will bring the process back. Under PM2 that
  // is autorestart; without a supervisor, "restart" is just an exit, and the
  // server never returns -- from a phone, with no way to start it again. So
  // the controls refuse rather than strand the operator.
  const SUPERVISED = !!(process.env.pm_id || config.pm2Name);
  ...
  return {
    ...
    restart(ip) {
      if (!SUPERVISED) {
        return { ok: false, reason: 'Not running under PM2 -- a restart would not come back.' };
      }
    ...
    startUpdate(ip) {
      if (!SUPERVISED || !PM2_NAME) {
        // Refusing beats guessing: update.sh ends in a PM2 restart, and a wrong
        // name restarts somebody else's server.
        return { ok: false, reason: 'Cannot tell which PM2 process this is -- set pm2Name in config.json.' };
      }
      ...
      const payload = Buffer.from(JSON.stringify({
        ...
        // The PID that HOLDS node-pty. update.sh proves the stop worked by
        // checking this is gone rather than trusting the pm2 exit code -- a
        // misconfigured pm2Name would otherwise let it install underneath a
        // live server, which is the damage the stop exists to prevent.
