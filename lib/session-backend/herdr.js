...
module.exports = function createHerdrBackend({ config, audit, getSafeEnv, pty, setLastError }) {
  ...
  return {
    ...
    //
    // It DOES wait for the pane to be roughly the size the client asked for.
    // `herdr workspace create` takes no --cols/--rows -- a pane is sized by the
    // clients attached to it -- so the sequence is: create the pane, attach the
    // pty (which sizes it), then start Claude. Measured at iPad 13 landscape,
    // the pane reached its 48 rows 1.5s before Claude first painted, so this
    // wait is already satisfied in practice. It is here so that margin is a
    // stated precondition rather than an accident of how many CLI round trips
    // happen to precede it: starting Claude into an 80x24 pane and resizing it
    // afterwards is a visible reflow on someone's screen.
    // Only ROWS are waited on, and that is a property of herdr's API rather
    // than an oversight: `pane get` reports `viewport_rows` and no column
    // count at all. It is still sufficient. A pty resize carries both
    // dimensions in a single event, so the pane's rows changing IS the
    // observation that the resize landed -- there is no path where herdr
    // applies the rows and drops the columns. The columns are confirmed
    // separately, from outside, by test/herdr-pane-geometry.py.
    launchClaude(proc, id, dims) {
      const name = sessionName(id);
      const wantRows = (dims && dims.rows) || 0;
      (async () => {
        ...
        // Tolerance, not equality: herdr spends a row or two on chrome
        // depending on width, so the pane is never exactly the pty's rows.
        if (wantRows) {
          const sizeBy = Date.now() + 8000;
          let sized = false;
          while (Date.now() < sizeBy && !sized) {
            try {
              const g = cli(['pane', 'get', paneId], { HERDR_SESSION: name });
              const rows = parseInt((g.match(/"viewport_rows":(\d+)/) || [])[1], 10);
              sized = Number.isFinite(rows) && rows >= wantRows - 4;
            } catch (e) { /* keep waiting */ }
            if (!sized) await sleep(POLL_MS);
          }
          // Timing out is not a reason to leave the session without Claude --
          // a mis-sized pane reflows, an empty one never starts.
          if (!sized) {
            audit('WARN', `${name}: pane never reached ~${wantRows} rows; starting Claude anyway`);
          }
        }
