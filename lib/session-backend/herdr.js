...
const { createAgentFeed } = require('../herdr-events');
...
module.exports = function createHerdrBackend({ config, audit, getSafeEnv, pty, setLastError }) {
  ...
  return {
    ...
    async create(id, { dir, cols, rows }) {
      ...
      try {
        // Orphaned so PM2's tree-kill cannot take the session down with the
        // server -- see lib/orphan-spawn.js for why detached alone is not
        // enough on Windows.
        //
        // The whitelist is applied HERE, once, and inherited down the chain:
        // launcher -> herdr server -> pane -> Claude. Handing this the
        // unfiltered process.env silently defeated ENV_WHITELIST for the only
        // process that matters, since the herdr SERVER owns the panes.
        orphanSpawn({
          command: BIN, args: ['server'], cwd: dir,
    ...
    // D7: the agent-status feed for this session (lib/herdr-events.js). Only
    // herdr has one -- the server treats a backend without agentFeed as
    // "attention comes from the regex detector". The socket path is read from
    // herdr, never derived, and is stable for a session name across restarts,
    // so it is resolved once here. Throws when herdr cannot be asked or the
    // session has no record; the caller retries.
    agentFeed(id, onChange) {
      const name = sessionName(id);
      const rec = recordFor(id, listSessions());
      if (!rec) throw new Error(`no herdr session record for ${name}`);
      if (typeof rec.socket_path !== 'string' || !rec.socket_path) {
        throw new Error(`herdr session ${name} reports no socket_path`);
      }
      return createAgentFeed({ socketPath: rec.socket_path, label: name, audit, onChange });
    },
