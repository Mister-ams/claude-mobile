      ...
      (async () => {
        ...
        const readRows = () => {
          try {
            ...
            const n = parseInt((g.match(/"viewport_rows":(\d+)/) || [])[1], 10);
        ...
        while (Date.now() < sizeBy && !settled) {
          await sleep(POLL_MS);
          const now = readRows();
          settled = now !== null && now === prev;
          prev = now;
        }
        // Timing out is not a reason to leave the session without Claude: a
        // mis-sized pane reflows, an empty one never starts.
        if (!settled) {
          audit('WARN', `${name}: pane geometry still changing after 8s; starting Claude anyway`);
        }
